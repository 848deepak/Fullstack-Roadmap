...
[Home](../../README.md) > 10_Authentication_and_Security > 04_mini_project
...
1. In this auth mini-project, where should identity provider concerns end and application authorization concerns begin?
2. How would you roll out MFA and step-up checks for sensitive actions without locking out legitimate users?
3. Which session/token lifecycle events must be auditable, and how would you design that audit trail?
4. How do you support account recovery securely while minimizing social-engineering abuse risk?
5. What deployment checklist must pass before exposing this auth service to production traffic?

### Production Best Practices Upgrade
- Define auth project boundaries clearly: identity proofing, token service, permission service, and policy enforcement points.
- Treat credential and token storage as critical assets with strict encryption-at-rest, rotation, and access controls.
- Include operational runbooks for account compromise, credential stuffing waves, and emergency key revocation.
- Validate abuse defenses with replay simulations, brute-force tests, and permission bypass attempts before launch.
- Gate production rollout with security review, logging validation, and incident drill completion.
