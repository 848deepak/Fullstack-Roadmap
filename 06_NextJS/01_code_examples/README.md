# Code Examples - 06_NextJS

This folder contains practical, production-oriented sample code.

## Included example
- 01_code_examples/app_router_server_component.tsx
...
[Home](../../README.md) > 06_NextJS > 01_code_examples
...
1. In these examples, how do you decide what stays in a Server Component versus what must be marked `use client`?
2. How would you explain a route waterfall and then fix it with parallel fetching or streaming?
3. Which example-level mistakes create hidden cache bugs even when the UI appears correct?
4. How do you verify an example is SEO-safe and hydration-safe before sharing it as a reference?
5. What minimal instrumentation should each example include to prove runtime behavior?

### Production Best Practices Upgrade
- Keep each example focused on one behavior (rendering mode, caching, routing, or boundary placement).
- Pair every example with one negative case (stale data, hydration edge, or permission failure).
- Add brief verification steps: expected HTML output, expected cache behavior, expected client bundle impact.
- Maintain explicit notes on what must never be moved from server to client.
...
- `06_NextJS/01_code_examples/app_router_server_component.tsx`
