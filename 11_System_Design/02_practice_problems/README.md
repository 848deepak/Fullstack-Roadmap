...
[Home](../../README.md) > 11_System_Design > 02_practice_problems
...
1. A practice design asks for global scale. Which assumptions do you challenge first before selecting technologies?
2. How would you choose between queue-based async workflows and direct RPC for a user-facing latency-sensitive feature?
3. In a scaling exercise, what metrics justify introducing caching tiers, and what invalidation risks follow?
4. How would you redesign a system that has acceptable average latency but poor p99 under burst traffic?
5. What is your process for validating that a proposed architecture actually meets reliability goals under fault scenarios?

### Production Best Practices Upgrade
- Define practice constraints explicitly: traffic model, consistency target, latency objective, and failure assumptions.
- Evaluate solutions with trade-off scoring (complexity, cost, operability, resilience), not only diagram completeness.
- Require failure-mode analysis and recovery steps as part of each design answer.
- Use iterative design progression (baseline → bottleneck analysis → improved architecture) to train structured thinking.
- Maintain a checklist for anti-patterns (single points of failure, unbounded queues, hidden coupling, missing backpressure).
