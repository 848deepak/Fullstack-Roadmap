# Code Examples - 11_System_Design

This folder contains practical, production-oriented sample code.

## Included example
- 01_code_examples/token_bucket_rate_limiter.js
...
[Home](../../README.md) > 11_System_Design > 01_code_examples
...
1. In this rate-limiter example, how do you decide local in-memory limiting vs centralized distributed limiting?
2. What correctness assumptions must hold for this example under multi-instance deployment and clock skew?
3. How would you explain burst handling and fairness trade-offs in token-bucket style controls?
4. Which parts of this example are educational shortcuts that should never ship as-is in production?
5. How do you instrument this pattern so operators can distinguish normal throttling from service distress?

### Production Best Practices Upgrade
- Keep examples focused, but annotate the production concerns they abstract away (state distribution, partial failure, and noisy-neighbor impact).
- Add explicit assumptions and limits to each example so learners know when the pattern breaks.
- Include test cases for overload, dependency latency, and intermittent failures in addition to happy paths.
- Tie example metrics to real SLO thinking (rejected requests, queue depth, saturation time, fallback activation).
- Keep pattern examples consistent with modern resilience and observability practices used in production platforms.
...
- `11_System_Design/01_code_examples/token_bucket_rate_limiter.js`
