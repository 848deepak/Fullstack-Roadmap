...
[Home](../../README.md) > 08_APIs_and_HTTP > 02_practice_problems
...
1. A practice task asks you to refactor inconsistent endpoints. Which contract rules would you enforce first to maximize client stability?
2. How would you solve a problem where repeated client retries create duplicate resources under intermittent network failures?
3. In pagination exercises, what signals indicate offset-based pagination is no longer sufficient and cursor pagination is needed?
4. How would you validate and benchmark a proposed caching strategy to ensure it helps p95 latency without serving stale critical data?
5. If two teams implement different error formats, how would you guide convergence with minimal migration disruption?

### Production Best Practices Upgrade
- Define clear acceptance criteria for each problem (status correctness, schema validity, latency budget, and failure behavior) before coding.
- Add scenario-driven test inputs that include malformed payloads, auth failures, retries, and race conditions to build production reflexes.
- Encourage solution writeups that justify trade-offs, not just passing outputs, especially for versioning and caching decisions.
- Reuse shared checklists for API consistency (naming, pagination, errors, idempotency, observability) across all practice sets.
- Track common failure patterns from solved problems and convert them into preventive patterns for later modules.
