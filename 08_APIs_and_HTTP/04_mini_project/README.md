...
[Home](../../README.md) > 08_APIs_and_HTTP > 04_mini_project
...
1. In this project, where do you place ownership boundaries between API gateway policies, service logic, and data integrity guarantees?
2. How would you design a release plan for introducing a new required field without breaking old clients still on previous contract versions?
3. Which endpoints in this project require idempotency guarantees, and how would you test those guarantees under retry storms?
4. If partner integrations have variable reliability, how would you isolate failures while preserving core API responsiveness?
5. What operational readiness checks must pass before promoting this API project to production traffic?

### Production Best Practices Upgrade
- Treat mini-project contracts as product contracts: version fields intentionally, publish change logs, and document migration paths.
- Add pre-release compatibility tests that replay representative client traffic against candidate builds.
- Implement resilience defaults for external dependencies (timeouts, bounded retries, circuit breakers, dead-letter handling).
- Enforce API governance checks in CI: schema diffs, linted naming conventions, and error-format consistency.
- Define and rehearse rollback procedures for gateway config, schema changes, and stateful idempotency stores.
